...
tax.textContent = (+(subtotal.textContent).replace("$", "")* 0.18).toFixed(2)+"$"
total_price.textContent = +(subtotal.textContent).replace("$", "")+ +(tax.textContent.replace("$", ""))+ +(shipping_price.textContent.replace("$", ""))+"$"
...
increase.forEach((item) => {
  item.addEventListener("click", () => {
    ...
    total_price.textContent = (+subtotal.textContent.replace("$", "") + +tax.textContent.replace("$", "") + +shipping_price.textContent.replace("$", "")).toFixed(2) +"$";
  });
});
...
decrease.forEach((item) => {
  item.addEventListener("click", () => {
    ...
    item.parentElement.nextElementSibling.nextElementSibling.children[0].textContent = (item.nextElementSibling.textContent * item.parentElement.previousElementSibling.children[0].textContent).toFixed(2);
